...
type DeviceLocation struct {
	...
	AccuracyGateways int32     `db:"accuracy_gateways" `
}
...
func buildDeviceLocationColumns(alias string) deviceLocationColumns {
	return deviceLocationColumns{
		ColumnsExpr: expr.NewColumnsExpr(
			"time", "device_id", "device_eui", "organization_slug", "latitude", "longitude", "accuracy_gateways",
		).WithParent("device_locations"),
		tableAlias:       alias,
		Time:             psql.Quote(alias, "time"),
		DeviceID:         psql.Quote(alias, "device_id"),
		DeviceEui:        psql.Quote(alias, "device_eui"),
		...
		AccuracyGateways: psql.Quote(alias, "accuracy_gateways"),
	}
}
...
type DeviceLocationSetter struct {
	...
	DeviceEui        omit.Val[string]    `db:"device_eui" `
	...
	AccuracyGateways omit.Val[int32]     `db:"accuracy_gateways" `
}
...
func (s DeviceLocationSetter) SetColumns() []string {
	...
	if s.DeviceEui.IsValue() {
		vals = append(vals, "device_eui")
	}
	...
	if s.AccuracyGateways.IsValue() {
		vals = append(vals, "accuracy_gateways")
	}
	return vals
}
...
func (s DeviceLocationSetter) Overwrite(t *DeviceLocation) {
	...
	if s.DeviceEui.IsValue() {
		t.DeviceEui = s.DeviceEui.MustGet()
	}
	if s.OrganizationSlug.IsValue() {
	...
	if s.AccuracyGateways.IsValue() {
		t.AccuracyGateways = s.AccuracyGateways.MustGet()
	}
}
...
func (s *DeviceLocationSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.DeviceEui.IsValue() {
			vals[2] = psql.Arg(s.DeviceEui.MustGet())
		} else {
			vals[2] = psql.Raw("DEFAULT")
		}

		if s.OrganizationSlug.IsValue() {
			vals[3] = psql.Arg(s.OrganizationSlug.MustGet())
		} else {
		...
		if s.AccuracyGateways.IsValue() {
			vals[6] = psql.Arg(s.AccuracyGateways.MustGet())
		} else {
...
func (s DeviceLocationSetter) Expressions(prefix ...string) []bob.Expression {
	...
	if s.DeviceEui.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "device_eui")...),
			psql.Arg(s.DeviceEui),
		}})
	}
	...
	if s.AccuracyGateways.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "accuracy_gateways")...),
			psql.Arg(s.AccuracyGateways),
		}})
	}
...
func buildDeviceLocationWhere[Q psql.Filterable](cols deviceLocationColumns) deviceLocationWhere[Q] {
	return deviceLocationWhere[Q]{
		...
		DeviceEui:        psql.Where[Q, string](cols.DeviceEui),
		...
		AccuracyGateways: psql.Where[Q, int32](cols.AccuracyGateways),
	}
}
