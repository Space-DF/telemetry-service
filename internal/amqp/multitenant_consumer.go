...
func NewMultiTenantConsumer(cfg config.AMQP, orgEventsCfg config.OrgEvents, processor MessageProcessor, logger *zap.Logger) *MultiTenantConsumer {
		...
		case msg, ok := <-messages:
			...
			if err := json.Unmarshal(msg.Body, &deviceMsg); err != nil {
				c.logger.Error("Failed to unmarshal message",
					zap.Error(err),
					zap.String("org", tenant.OrgSlug))
				_ = msg.Nack(false, false) // Don't requeue bad messages
				continue
			}
			...
			if err := c.processor.ProcessMessage(ctx, &deviceMsg); err != nil {
				c.logger.Error("Failed to process message",
					zap.Error(err),
					zap.String("org", tenant.OrgSlug),
					zap.String("device_eui", deviceMsg.DeviceEUI))
