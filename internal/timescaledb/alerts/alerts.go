...
func (s *Service) GetAlerts(ctx context.Context, orgSlug, category, spaceSlug, deviceID, dateStr string, cautionThreshold, warningThreshold, criticalThreshold float64, page, pageSize int) ([]interface{}, int, error) {
	org := orgSlug
	if org == "" {
...
func buildDateRange(dateStr string) (time.Time, time.Time, error) {
	...
	if trimmed == "" {
		return time.Time{}, time.Time{}, fmt.Errorf("%w", core.ErrDateRequired)
	}
	...
	if err != nil {
		return time.Time{}, time.Time{}, fmt.Errorf("%w", core.ErrInvalidDateFormat)
	}
